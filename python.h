...
/* takes an array. sorts, and removes all duplicates
   returns new length of array */
int set(void *values, int len, size_t size);

/* takes an array of values and a container array, and transfers all unqiue values of values into container (sorted order)
...
/* takes two arrays and a container and joins both together (array1 + array2) into container
   returns container length */
int join(void *values1, void *values2, int len1, int len2, void *container, size_t size);


/* performs union (join) of two arrays into container, then makes a set of the container
   returns container length */
int inboth(void *values1, void *values2, int len1, int len2, void *container, size_t size);


/* takes two sets and container. sorts both, fills container with set1 values which do not exist in set2 (sorted order)
   returns contianer length */
int diff(void *values1, void *values2, int len1, int len2, void *container, size_t size);

/* takes two arrays and container. creates sorted sets of both, fills container with array1 values which do not exist in array2 (sorted order)
   returns contianer length */
int diff_arr(void *values1, void *values2, int len1, int len2, void *container, size_t size);


/* takes two sets and a container. sorts both, fills container with symetric difference of set1 and set2
   returns container length */
int symdiff(void *values1, void *values2, int len1, int len2, void *container, size_t size);

/* takes two arrays and a container. fills container with symetric difference of array1 and array2
   returns container length */
int symdiff_arr(void *values1, void *values2, int len1, int len2, void *container, size_t size);


/* takes two sets and container. sorts both, fills container (in order of set1) with intersecting values
   returns length of container */
int inter(void *values1, void *values2, int len1, int len2, void *container, size_t size);

/* takes two arrays and container. creates sorted sets of both, fills container (in order of array1) with intersecting values
   returns length of container */
